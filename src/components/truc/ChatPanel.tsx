...
export function ChatPanel({ onSay, highlightPreguntes, highlightRespostes, highlightAltres, hiddenPhraseIds }: ChatPanelProps) {
  const t = useT();
  ...
  return (
    <>
      {(Object.keys(GROUPS) as GroupKey[]).map((key) => {
        ...
        return (
          <button
            ...
            aria-label={g.label}
            className={cn(
              "fixed right-3 z-40 w-12 h-12 rounded-full",
              "shadow-lg",
      ...
      {active && (
        <div className="fixed inset-x-0 bottom-0 z-30 animate-fade-in">
          <div
...
export function ChatBubble({
  ...
  return (
    <div
      className={cn(
        "absolute z-[60] px-3 py-1.5 rounded-2xl pointer-events-none",
