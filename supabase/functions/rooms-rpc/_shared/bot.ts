...
export interface BotHints {
   ...
   * (vine-a-mi o tinc-bona). Quan és `true`, mai es reserva una carta
   * forta confiant que la mesa és inofensiva.
   */
...
export function botDecide(
  ...
  // reals de guanyar l'envit (≥31). Sense farols ni envits especulatius amb 27/30.
  if (canEnvit && bluffRate === 0) {
    // Mode sincer: amb 31/32/33 d'envit sempre envida (mà o peu).
...
function decideEnvitResponse(
  ...
  else { win = 8; lose = 8; costRebuig = 2; }
  ...
  //   (a) myEnvit ≥ 31, o
  //   (b) el meu equip ja ha guanyat la primera baza I tinc carta top
  ...
  //       company té carta top (partnerAdvice === "strong").
  ...
  // · myEnvit ≥ 31 → mai "no-vull" en sincer.
  // · myEnvit ≥ 32 i nivell 2 (envit) → "renvit".
...
function decideTrucResponse(
  ...
  // Suma total de força de la mà (màxim teòric ~270 amb les 3 cartes top més altes).
...
function decideProactiveTruc(
  ...
  const topCards = hand.filter(c => cardStrength(c as any) >= 80).length;
  const goodCards = hand.filter(c => cardStrength(c as any) >= 60).length;
  // "Top de truc" segons l'usuari: 7 oros, 7 espases, As bastos, As espases.
  const hasTopTrucCard = hand.some(c =>
    (c.rank === 7 && (c.suit === "oros" || c.suit === "espases")) ||
    (c.rank === 1 && (c.suit === "bastos" || c.suit === "espases")),
  );
  ...
  if (veryStrongHand && !closeToWin) {
    ...
    if (bluffRate === 0) {
      ...
      // si va perdent per molt. Així no s'abusa del "Truc i passe".
  ...
  // forta de truc (7 oros / 7 espases / As bastos / As espases) i, a més,
  // l'equip ja ha guanyat la 1a baza o el company ha confirmat força. A
  // més, per no abusar del "Truc i passe", només truca proactivament si va
  // perdent per molt o està a punt de tancar la cama; en cas contrari
  // espera que truque el rival per poder retrucar i guanyar més punts.
  if (bluffRate === 0) {
    const partnerStrong = partnerAdvice === "strong";
